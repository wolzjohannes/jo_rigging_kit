...
def point_on_curve_weights(cvs, t, degree, knots=None):
    """
    ...
    Args:
        cvs(list, tuple): A list of cvs, these are used for the return value.
...
def tangent_on_curve_weights(cvs, t, degree, knots=None):
    """
...
def pointOnSurfaceWeights(cvs, u, v, uKnots=None, vKnots=None, degree=3):
    ...
    Args:
        cvs(list, tuple): A list of cv rows, these are used for the return value.
...
def tangentUOnSurfaceWeights(cvs, u, v, uKnots=None, vKnots=None, degree=3):
    ...
    matrix_weight_rows = [point_on_curve_weights(row, u, degree, uKnots) for row in cvs]
    matrix_weight_columns = tangent_on_curve_weights([i for i in range(len(matrix_weight_rows))], v, degree, vKnots)
    surface_matrix_weights = []
    for index, weight in matrix_weight_columns:
        matrix_weights = matrix_weight_rows[index]
        surface_matrix_weights.extend([[m, (w * weight)] for m, w in matrix_weights])

    return surface_matrix_weights
...
def _test_joint(radius=1.0, name='sphere'):
    """ Creates a joint for testing purposes. """
    jnt_nde = cmds.createNode("joint", name=name)
    cmds.setAttr("{0}.radius".format(name), radius)
...
def _testMatrixOnCurve(count=4, pCount=None, degree=3):
    ...
    for i in range(pCount):
        t = i / (float(pCount) - 1)
        pNode = _test_joint(pRadius, name='p%s' % i)

        # Create the position matrix
        pointMatrixWeights = point_on_curve_weights(cvMatrices, t, degree=degree)
        ...
        tangentMatrixWeights = tangent_on_curve_weights(cvMatrices, t, degree=degree)
...
def _testMatrixOnCircularCurve(count=4, pCount=None, degree=3):
    ...
    for i in range(pCount):
        ...
        # Create the position matrix
        pointMatrixWeights = point_on_curve_weights(cvMatrices, t, degree=degree, knots=knots)
        pointMatrixNode = cmds.createNode('wtAddMatrix', name='pointMatrix0%s' % (i + 1))
        pointMatrix = '%s.matrixSum' % pointMatrixNode
        for index, (matrix, weight) in enumerate(pointMatrixWeights):
            cmds.connectAttr(matrix, '%s.wtMatrix[%s].matrixIn' % (pointMatrixNode, index))
        ...
        # Create the tangent matrix
        tangentMatrixWeights = tangent_on_curve_weights(cvMatrices, t, degree=degree, knots=knots)
        tangentMatrixNode = cmds.createNode('wtAddMatrix', name='tangentMatrix0%s' % (i + 1))
        tangentMatrix = '%s.matrixSum' % tangentMatrixNode
        for index, (matrix, weight) in enumerate(tangentMatrixWeights):
            cmds.connectAttr(matrix, '%s.wtMatrix[%s].matrixIn' % (tangentMatrixNode, index))
...
def _test_matrix_on_surface(u_count=3,
                            v_count=3,
                            degree=2
                            ):
    ...
    Args:
        u_count(int): The amount of cvs in u.
        v_count(int): The amount of cvs in v.
        degree(int): The degree of the curve.
    """
...
def create_hh_ribbon(controllers: Union[list, tuple],
                     curve_points: Optional[int] = None,
                     degree: int = 3,
                     name: Optional[str] = None,
                     add_data_to: Optional[str] = None,
                     output_objects: Optional[Union[list, tuple]] = None,
                     global_scale: Optional[str] = None
                     ) -> tuple:
    """
    Creates an example curve with the given cv and point counts.

    Args:
        controllers(list, tuple): The amount of cvs.
        curve_points(int): The amount of points to attach to the curve.
        degree(int): The degree of the curve.
        name(str): the name of the system.
        add_data_to(str, None): the name of the node where we keep the deboor data.
        output_objects(list, tuple, None):
        global_scale(str, None): if  there is a global scale attribute given, this will add the global scale back ontop.

    Returns:
        tuple: list of attribtues.

    """
    name_ = name or "default_deboor"

    controller_amount = len(controllers)
    curve_points = curve_points or controller_amount * 2
    p_radius = 0.5

    output_objects_ = output_objects or tuple(_test_joint(p_radius, name=f'{name_}_{iteration_:03}_JNT')
                                              for iteration_
                                              in range(curve_points)
                                              )

    # Create the control points
    cv_matrices = tuple(f"{controller_}.worldMatrix[0]" if "." not in controller_ else controller_
                        for controller_ in controllers)

    # Attach the cubes
    for iteration_ in range(curve_points):
        t = iteration_ / (float(curve_points) - 1.0)

        p_node = output_objects_[iteration_]

        # Create the position matrix
        point_matrix_weights = point_on_curve_weights(cv_matrices, t, degree=degree)
        point_matrix_name = cmds.createNode('wtAddMatrix',
                                            name=f'{name_}_pointMatrix_{iteration_:03}_WAM'
                                            )

        point_matrix = f'{point_matrix_name}.matrixSum'
        for index, (matrix, weight) in enumerate(point_matrix_weights):
            cmds.connectAttr(matrix, f'{point_matrix_name}.wtMatrix[{index}].matrixIn')
            cmds.setAttr(f'{point_matrix_name}.wtMatrix[{index}].weightIn', weight)

            if add_data_to:
                cmds.addAttr(add_data_to, ln=f"{name_}_pointMatrix_{iteration_:03}_weightIndex_{index:03}", at="double")
                cmds.setAttr(f"{add_data_to}.{name_}_pointMatrix_{iteration_:03}_weightIndex_{index:03}", weight)

                cmds.connectAttr(f"{add_data_to}.{name_}_pointMatrix_{iteration_:03}_weightIndex_{index:03}",
                                 f"{point_matrix_name}.wtMatrix[{index}].weightIn",
                                 f=True
                                 )

        # Create the tangent matrix
        tangent_matrix_weights = tangent_on_curve_weights(cv_matrices, t, degree=degree)
        tangent_matrix_name = cmds.createNode('wtAddMatrix', name=f'{name_}_tangentMatrix_{iteration_:03}_WAM'
                                              )

        tangent_matrix = f'{tangent_matrix_name}.matrixSum'
        for index, (matrix, weight) in enumerate(tangent_matrix_weights):
            cmds.connectAttr(matrix, f'{tangent_matrix_name}.wtMatrix[{index}].matrixIn')
            cmds.setAttr(f'{tangent_matrix_name}.wtMatrix[{index}].weightIn', weight)

            if add_data_to:
                cmds.addAttr(add_data_to, ln=f"{name_}_tangentMatrix_{iteration_:03}_weightIndex_{index:03}",
                             at="double"
                             )

                cmds.setAttr(f"{add_data_to}.{name_}_tangentMatrix_{iteration_:03}_weightIndex_{index:03}", weight)

                cmds.connectAttr(f"{add_data_to}.{name_}_tangentMatrix_{iteration_:03}_weightIndex_{index:03}",
                                 f"{tangent_matrix_name}.wtMatrix[{index}].weightIn",
                                 f=True
                                 )
        ...
        # Create an aim matrix node
        aim_matrix_name = cmds.createNode('aimMatrix', name=f'{name_}_aimMatrix_{iteration_:03}_AMX')
        cmds.connectAttr(point_matrix, f'{aim_matrix_name}.inputMatrix')
        cmds.connectAttr(tangent_matrix, f'{aim_matrix_name}.primaryTargetMatrix')

        cmds.setAttr(f'{aim_matrix_name}.primaryMode', 1)
        cmds.setAttr(f'{aim_matrix_name}.primaryInputAxis', 1, 0, 0)
        cmds.setAttr(f'{aim_matrix_name}.secondaryInputAxis', 0, 1, 0)
        cmds.setAttr(f'{aim_matrix_name}.secondaryMode', 0)

        aim_matrix_output = f'{aim_matrix_name}.outputMatrix'

        # Remove scale
        pick_matrix_name = cmds.createNode('pickMatrix', name=f'{name_}_noScale_{iteration_:03}_PMX')
        cmds.connectAttr(aim_matrix_output, f'{pick_matrix_name}.inputMatrix')
        cmds.setAttr(f'{pick_matrix_name}.useScale', False)
        cmds.setAttr(f'{pick_matrix_name}.useShear', False)

        output_matrix_name = f'{pick_matrix_name}.outputMatrix'

        if global_scale:
            global_scale_name = cmds.createNode("math_MatrixFromTRS", name=f'{name_}_addScale_{iteration_:03}_TMX')

            for axis_ in "XYZ":
                cmds.connectAttr(global_scale, f"{global_scale_name}.scale{axis_}")

            scaling_addition = cmds.createNode("math_MultiplyMatrix", name=f'{name_}_addScale_{iteration_:03}_MMX')

            cmds.connectAttr(f"{global_scale_name}.output",
                             f"{scaling_addition}.input1")

            cmds.connectAttr(f"{pick_matrix_name}.outputMatrix",
                             f"{scaling_addition}.input2")

            output_matrix_name = f"{scaling_addition}.output"

        cmds.connectAttr(output_matrix_name, f'{p_node}.offsetParentMatrix')

    return output_objects_
