...
from pxo_rigging_kit.maya_utils.rigging.legacy_rig_lib.modules import rivetControls
from ...modules import freeControls
from ...systems import facialSystem
import pymel.core as pm


class Arrax_face_rig(facialSystem.Facial_system):
    FREECTLS_JNTS = (
        "eyeOrbicularis_R_0_start_default_faceJnt",
        "eyeOrbicularis_L_0_start_default_faceJnt",
        "throat_C_0_start_default_faceJnt",
    )
    ORBICULARIS_PARENT = "C_bnd_upperJaw_0_0_faceJnt"
    THIRD_EYELID_JNTS = (
        "eyelidThird_R_0_start_default_localJnt",
    ...
    def __init__(self):
        ...
        model_path = r"X:\redgun_reg-6344\_library\assets\creature\crt_arrax\mdl\_publish\reg_crt_arrax_mdl_v094_awg.mb"
        super(Arrax_face_rig, self).__init__(
            ...
            load_ctls=1,
            load_skin=1,
            pre_build=self.arrax_face_pre_build,
            upgrade=self.arrax_face_upgrade,
            post_build=self.arrax_face_post_build,
        )

    def arrax_face_pre_build(self):
        ...
        global_elements = [
            "{}EyefluidLower_C_001_high_geo".format(self.name),
            ...
            "{}Horns_C_001_high_geo".format(self.name),
        ]

        for element in global_elements:
            element = pm.PyNode(element)
            element_dup = pm.duplicate(
                element, n=element.name().replace("_high_", "_local_")
            )[0]
            pm.parent([element_dup], "no_transf_GRP")
            pm.blendShape(
                [element_dup], element, frontOfChain=True, weight=(0, 1)
            )

    def arrax_face_upgrade(self):
        self.eye_orbicularis_ctl()
        self.headSpikes_rivet_build()

    def arrax_face_post_build(self):
        ...
        elements_to_delete = [
            ...
            "{}Scales_C_001_high_grp".format(self.name),
            "{}Body_C_high_grp".format(self.name),
    ...
    def eye_in_shift(self):
        ...
        jnts_list = (
            "eyeIn_R_0_end_default_localJnt",
            "eyeIn_L_0_end_default_localJnt",
        )
    ...
    def eye_orbicularis_ctl(self):
        freeCtls_mod = freeControls.FreeControls(
            name="Main",
            scale=10.0,
    ...
    def iris_connections(self):
        if pm.objExists("irisBlendshape_grp"):
            for e in pm.listRelatives("irisBlendshape_grp"):
                blend_shape_nd = pm.blendShape(
                    [e],
                    "{}Iris_C_001_local_geo".format(self.name),
                    frontOfChain=True,
                    weight=(0, 1),
                )
                side = name.get_side(e, with_undescore=0)
                pm.addAttr(
                    "eyelid_{}_control_default_ctrl".format(side),
                    ln="iris",
                    type="float",
                    dv=0.0,
                    min=0.0,
                ...
                pm.connectAttr(
                    "eyelid_{}_control_default_ctrl.iris".format(side),
    ...
    def third_eyelid_rig(self):
        for e in self.THIRD_EYELID_JNTS:
            ...
            pm.addAttr(
                ctl, ln="eyelid", type="float", dv=0.0, min=0.0, max=1.0, k=1
            )
            pnodes.create_remap_val_node(
                input="{}.eyelid".format(ctl),
                inputMin=0,
                inputMax=1,
                outputMin=0,
                outputMax=110,
    ...
    def throat_connections(self):
        if pm.objExists("throatBS_grp"):
            pm.addAttr(
                "throatMain0_C_control_default_ctrl",
                ln="contract",
                type="float",
                dv=0.0,
                min=-1.0,
            ...
            static_mesh = pm.duplicate(
                "{}Gums_C_001_high_geo".format(self.name),
                n="{}Gums_C_001_local_geo".format(self.name),
            )[0]
            pm.parent(static_mesh, "no_transf_GRP")
            blend_shape_nd = pm.blendShape(
                pm.listRelatives("throatBS_grp"),
                static_mesh,
                frontOfChain=True,
                weight=(0, 1),
            )
            ...
            for e in ("throatContraction_geo", "throatExpansion_geo"):
                ...
                pnodes.create_remap_val_node(
                    input="throatMain0_C_control_default_ctrl.contract",
                    inputMin=input_min,
                    inputMax=input_max,
                    outputMin=0,
                    outputMax=1,
            ...
            pm.connectAttr(
                "facial_BS.L_smile", "{}.L_gumSmile".format(blend_shape_nd[0])
            )
            pm.connectAttr(
                "facial_BS.R_smile", "{}.R_gumSmile".format(blend_shape_nd[0])
            )

    def headSpikes_rivet_build(self):
        pm.parent("snapMeshRivetBS_geo", self.base_module.no_transf_grp)
        rivet_geo = pm.duplicate("snapMeshRivetBS_geo", n="snapMeshRivet_geo")
        tweaker_geo = pm.duplicate(
            "snapMeshRivetBS_geo", n="snapMeshRivetTweaker_geo"
        )
        bs_node = pm.blendShape(
            [tweaker_geo[0], "snapMeshRivetBS_geo"],
            rivet_geo,
            frontOfChain=True,
            weight=((0, 1), (1, 1)),
        )

        rivetControls.RivetControls(
            name="headSpikes",
            moving_mesh=rivet_geo[0],
            dummies_rivet_grp="headSpikes_rivets_grp",
            create_controls=0,
            seperate_hierarchy=0,
            base_module=self.base_module,
        )
        facialBS_targets = pm.listAttr("facial_BS.w", m=1)
        for t in facialBS_targets:
            try:
                pm.connectAttr(
                    "facial_BS.{}".format(t), "spikesBase_BS.{}".format(t), f=1
                )
            except:
                pm.warning(
                    "spikesBase_BS.{} not exist. Can not connect with facial_BS.{}".format(
                        t, t
                    )
                )
